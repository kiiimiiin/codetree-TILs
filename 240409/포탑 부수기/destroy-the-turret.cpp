...
tuple<int, int, int, int> ChooseTarget() { // 가장 약한 공격자, 강한 공격대상 선정
	vector<tuple<int, int, int, int>> infos; // 공격력, 공격 turn, 행열합,열
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			if (board[i][j].first == 0) continue;
			infos.push_back({ board[i][j].first, board[i][j].second
				, i + j, j });
		}
	}
	...
	int ax, ay, sum;
	tie(ignore, ignore, sum, ay) = infos.front();
	ax = sum - ay;

	int tx, ty;
	tie(ignore, ignore, sum, ty) = infos.back();
	tx = sum - ty;
	return { ax, ay , tx, ty };
}
...
int main(void) {
	...
	// 가장 강한 포탑 공격력 출력
	int tx, ty;
	tie(ignore, ignore, tx, ty) = ChooseTarget();
